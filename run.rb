...
ATSPIBus.new(logger: logger).with do
  KWin.with do
    Driver.with(datadir) do
      ...
      # temporarily send stdout to stderr, kwin currently doesn't correctly forward stderr
      # https://invent.kde.org/plasma/kwin/-/merge_requests/3595
      IO.popen(ARGV, 'r') { |io| warn io.readlines }
      ret = $?.success?
